...
function countVowels(str) {
  ...
  return [...str].reduce((count, char) => count + vowels.includes(char), 0);
}
...
function isPalindrome(str) {
  const sourceStr = str.replace(/[?!,\s]/g, '').toLowerCase();
  let j = sourceStr.length - 1;
  for (let i = 0; i < sourceStr.length; i += 1) {
    if (i === j) {
      break;
    }
    if (sourceStr.charAt(i) !== sourceStr.charAt(j)) {
      return false;
    }
    j -= 1;
  }
...
function encodeToRot13(str) {
  ...
  const subst = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return [...str]
    .map((char) =>
      alphabet.includes(char) ? subst.charAt(alphabet.indexOf(char)) : char
    )
    .join('');
}
